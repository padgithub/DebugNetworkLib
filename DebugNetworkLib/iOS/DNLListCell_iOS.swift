...
class DNLListCell: UITableViewCell
{
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?)
    {
        ...
        self.requestTimeLabel.font = UIFont.DNLFontBold(size: 13)
        ...
        self.timeIntervalLabel.font = UIFont.DNLFont(size: 12)
        contentView.addSubview(self.timeIntervalLabel)
        
        self.URLLabel = UILabel(frame: CGRect.zero)
        self.URLLabel.textColor = UIColor.DNLBlackColor()
        self.URLLabel.font = UIFont.DNLFont(size: 12)
        ...
        self.methodLabel.textColor = UIColor.DNLGray44Color()
        self.methodLabel.font = UIFont.DNLFont(size: 12)
        contentView.addSubview(self.methodLabel)
        
        self.typeLabel = UILabel(frame: CGRect.zero)
        self.typeLabel.textColor = UIColor.DNLGray44Color()
        self.typeLabel.font = UIFont.DNLFont(size: 12)
        contentView.addSubview(self.typeLabel)
        
        self.circleView = UIView(frame: CGRect.zero)
        self.circleView.backgroundColor = UIColor.DNLGray44Color()
        ...
        self.rightSeparator.backgroundColor = UIColor.DNLLightGrayColor()
    ...
    func configForObject(_ obj: DNLHTTPModel)
    {
    ...
    {
        if status == 999 {
            self.statusView.backgroundColor = UIColor.DNLGray44Color() //gray
            self.timeIntervalLabel.textColor = UIColor.white

        } else if status < 400 {
            self.statusView.backgroundColor = UIColor.DNLGreenColor() //green
            self.timeIntervalLabel.textColor = UIColor.DNLDarkGreenColor()

        } else {
            self.statusView.backgroundColor = UIColor.DNLRedColor() //red
            self.timeIntervalLabel.textColor = UIColor.DNLDarkRedColor()
        }
    }
    ...
    {
        if responseDate.isGreaterThanDate(DNL.sharedInstance().getLastVisitDate()) {
            self.isNew()
        } else {
            self.isOld()
        }
