...
@objc public class DNLHTTPModel: NSObject
{
    ...
    func saveRequest(_ request: URLRequest)
    {
        ...
        self.requestURL = request.getDNLURL()
        self.requestURLComponents = request.getDNLURLComponents()
        self.requestURLQueryItems = request.getDNLURLComponents()?.queryItems
        self.requestMethod = request.getDNLMethod()
        self.requestCachePolicy = request.getDNLCachePolicy()
        self.requestTimeout = request.getDNLTimeout()
        self.requestHeaders = request.getDNLHeaders()
    ...
    {
        saveRequestBodyData(request.getDNLBody())
    }
    
    func logRequest(_ request: URLRequest)
    {
        formattedRequestLogEntry().appendToFile(filePath: DNLPath.SessionLog)
    }
    ...
    func saveResponse(_ response: URLResponse, data: Data, urlSessionDataTask: URLSessionDataTask? = nil)
    {
        ...
        self.responseStatus = response.getDNLStatus()
        self.responseHeaders = response.getDNLHeaders()
        
        let headers = response.getDNLHeaders()
    ...
    {
        return String("DNL_request_body_") + "\(self.requestTime!)_\(getRandomHash() as String)"
    }
    ...
    {
        return String("DNL_response_body_") + "\(self.requestTime!)_\(getRandomHash() as String)"
    }
